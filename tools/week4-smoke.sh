...
PG_NAME="sprocket-v3-pg-week4-smoke"
...
for offset in 0 1 2 3 4; do
  ...
  if docker run --name "$PG_NAME" \
    ...
    -d postgres:16 >/dev/null 2>/tmp/sprocket_v3_week4_docker_err.log; then
...
if [[ -z "${DB_URL}" ]]; then
  ...
  cat /tmp/sprocket_v3_week4_docker_err.log >&2 || true
  exit 1
fi
...
DATABASE_URL="$DB_URL" MIGRATIONS_DIR="./migrations" go run ./cmd/migrate >/tmp/sprocket_v3_week4_migrate.log

echo "Starting API in DB-required mode"
DATABASE_URL="$DB_URL" REQUIRE_DATABASE=true go run ./cmd/api >/tmp/sprocket_v3_week4_api.log 2>&1 &
API_PID=$!
sleep 1
